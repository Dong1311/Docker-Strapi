...
const SellProducts = () => {
  ...
  const [change, setChange] = useState(0); // Tiền thừa trả khách
  const [revenueId] = useState(1); // ID của revenue để cập nhật (giả sử bạn biết ID của revenue)
  ...
  const handleCheckout = async () => {
    if (customerPayment >= totalAmount - discount) {
      // Cập nhật revenue trước khi thanh toán
      await updateRevenue();
