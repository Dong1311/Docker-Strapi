...
const Shop = ({ cartItems, updateCartItemQuantity, setProducts, products = [] }) => {
    const [page, setPage] = useState(1); 
    const [totalPages, setTotalPages] = useState(1); 
    const [language, setLanguage] = useState('en'); // Trạng thái ngôn ngữ, mặc định là tiếng Anh
    const pageSize = 6;

    // Hàm để gọi API sản phẩm theo ngôn ngữ và trang hiện tại
    const fetchProducts = useCallback(async () => {
        try {
            ...
            if (res.data && res.data.data) {
                setProducts(res.data.data); // Lưu danh sách sản phẩm
                setTotalPages(res.data.meta.pagination.pageCount); // Cập nhật số trang
            } else {
    ...
    // Gọi API mỗi khi ngôn ngữ hoặc trang thay đổi
    useEffect(() => {
        fetchProducts();
    }, [fetchProducts]);
